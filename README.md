...
-   **5 小时步调追踪**：状态栏以 `12% | 5%😊` 格式同时显示最高用量和 5h 用量进度，emoji 反映步调：😊 安全（低于时间进度）、😐 正常、😰 超支
...
-   **动画效果**：刷新时瓶子旋转 360° 后过冲回弹，用量变化时摇晃站稳
...
-   **状态栏**：`12% | 5%😊` 格式，左侧为最高用量周期百分比，右侧为 5h 用量及步调 emoji（😊 安全 / 😐 正常 / 😰 超支）
